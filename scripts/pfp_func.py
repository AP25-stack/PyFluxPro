...
def Convert_kgpm3_to_gpm3(ds, AH_out, AH_in):
    ...
    Usage:
     pfp_func.Convert_kgpm3_to_gpm3(ds, AH_out, AH_in)
    Author: PRI
    Date: August 2020
    """
    var_in = pfp_utils.GetVariable(ds, AH_in)
...
def Convert_mgCO2pm3_to_umolpmol(ds, MF_out, CO2_in, Ta_in, ps_in):
    """
    Purpose:
     Calculate CO2 mole fraction in uml/mol from mass density in mgCO2/m3.
    Usage:
     pfp_func.Convert_mgCO2pm3_to_umolpmol(ds, MF_out, CO2_in, Ta_in, ps_in)
    Author: PRI
    Date: August 2019
    """
    nRecs = int(ds.globalattributes["nc_nrecs"])
